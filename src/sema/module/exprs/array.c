...
SemaValue *sema_module_emit_expr_array(SemaModule *module, AstExprArray *array, SemaExprCtx ctx) {
    ...
    for (size_t i = 0; i < vec_len(array->elements); i++) {
        ...
        vec_push(elements, sema_module_expr_emit_runtime(module->mempool, runtime, ctx.output));
    }
    size_t step_id = sema_expr_output_push_step(ctx.output, ir_expr_step_new_array(sema_type_ir_id(type), elements));
