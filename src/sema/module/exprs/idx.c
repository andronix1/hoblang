...
SemaValue *sema_module_emit_expr_idx(SemaModule *module, AstExprIdx *idx, SemaExprCtx ctx) {
    ...
    SemaValueRuntime *idx_runtime = NOT_NULL(sema_module_emit_runtime_expr(module, idx->idx,
        sema_expr_ctx_new(ctx.output, NULL)));
    size_t value_step = sema_module_expr_emit_runtime(module->mempool, inner_runtime, ctx.output);
    size_t idx_step = sema_module_expr_emit_runtime(module->mempool, idx_runtime, ctx.output);
    SemaType *type = inner_runtime->type;
    if (type->kind == SEMA_TYPE_ARRAY) {
        size_t step_id = sema_expr_output_push_step(ctx.output, ir_expr_step_new_idx_array(value_step, idx_step));
    ...
    if (type->kind == SEMA_TYPE_POINTER) {
        size_t step_id = sema_expr_output_push_step(ctx.output, ir_expr_step_new_idx_pointer(value_step, idx_step));
