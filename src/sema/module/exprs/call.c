...
SemaValue *sema_module_emit_expr_call(SemaModule *module, AstCall *call, SemaExprCtx ctx) {
    ...
    if (vec_len(type->function.args) != vec_len(call->args) + is_ext) {
        sema_module_err(module, call->inner->slice, "expected to $l arguments to be passed but $l was passed",
            vec_len(type->function.args) - is_ext, vec_len(call->args));
        return NULL;
    }

    size_t callable = sema_module_expr_emit_runtime(module, runtime, ctx.output);
